...
function castSearchRows(){const q=(document.querySelector('[data-cast-search]')?.value||'').toLowerCase();return (BState.data.cast||[]).filter(c=>[c.CharacterName,c.PersonName,c.RoleLabel,c.World].join(' ').toLowerCase().includes(q));}
...
function renderCastRows(rows){return rows.map(c=>`<div class="blocking-person" draggable="${canEdit()?'true':'false'}" data-cast="${c.BlockingCastID}" title="Drag onto the stage"><span>${blockingCastAvatar(c,'small')}</span><span><span class="blocking-person-name">${BRM.escape(c.CharacterName)}</span><span class="blocking-person-role">${BRM.escape(c.PersonName||'Unassigned performer')}${c.RoleLabel?` · ${BRM.escape(c.RoleLabel)}`:''}</span></span>${canEdit()?'<span class="blocking-add">＋</span>':''}</div>`).join('')}
