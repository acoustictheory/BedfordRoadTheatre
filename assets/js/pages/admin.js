...
function renderAdmin(main,data){
  ...
  main.querySelectorAll('[data-quick-delete-user]').forEach(button=>button.addEventListener('click',()=>quickDeleteAdminUser(button,data)));
  main.querySelector('[data-user-search]')?.addEventListener('input',applyAdminUserFilters);
  main.querySelector('[data-user-department]')?.addEventListener('change',applyAdminUserFilters);
  main.querySelector('[data-user-status]')?.addEventListener('change',applyAdminUserFilters);
  main.querySelector('[data-user-access]')?.addEventListener('change',applyAdminUserFilters);
  main.querySelector('[data-user-sort]')?.addEventListener('change',applyAdminUserFilters);
...
function renderUsersTab(data){
  const departments=(data.departments||[]).slice().sort((a,b)=>String(a.Name).localeCompare(String(b.Name)));
  const rows=(data.users||[]).slice().sort((a,b)=>String(a.DisplayName||a.Username).localeCompare(String(b.DisplayName||b.Username)));
  ...
  <div class="people-grid" data-admin-users>${rows.map(u=>{
    const ids=Array.isArray(u.DepartmentIDs)?u.DepartmentIDs:[];
    const departmentNames=ids.map(id=>data.departments.find(d=>d.DepartmentID===id)?.Name||'').filter(Boolean);
    const deletable=!adminBool(u.IsFullAdmin)&&String(u.UserID)!==String(BRM.context?.userId);
    const pending=(data.departmentRequests||[]).filter(r=>r.UserID===u.UserID&&r.Status==='Pending').length;
    return `<article class="people-card" data-user-id="${BRM.escape(u.UserID)}" data-user-search-text="${BRM.escape(`${u.DisplayName||''} ${u.Username||''} ${u.Email||''}`.toLowerCase())}" data-user-departments="${BRM.escape(ids.join('|'))}" data-user-name="${BRM.escape(String(u.DisplayName||u.Username).toLowerCase())}" data-user-department-name="${BRM.escape(String(departmentNames[0]||'zzzz').toLowerCase())}" data-user-grade="${BRM.escape(String(u.Grade||'zzzz').toLowerCase())}" data-user-status="${BRM.escape(String(u.Status||''))}" data-user-admin="${adminBool(u.IsFullAdmin)}" data-user-password="${adminBool(u.MustChangePassword)}" data-user-created="${BRM.escape(String(u.CreatedAt||''))}">${BRM.avatar(u.DisplayName||u.Username,u.PhotoURL||'','large')}<div class="people-card-main"><h3>${BRM.escape(u.DisplayName||u.Username)}</h3><span class="field-hint">@${BRM.escape(u.Username)}${u.Grade?` · Grade ${BRM.escape(u.Grade)}`:''}</span><div class="people-card-meta"><span class="badge">${BRM.escape(u.Status)}</span>${adminBool(u.IsFullAdmin)?'<span class="badge badge-urgent">Full Admin</span>':'<span class="badge">Student</span>'}${adminBool(u.MustChangePassword)?'<span class="badge badge-warning">Password change</span>':''}${pending?`<a href="#requests" class="badge badge-important">${pending} request(s)</a>`:''}</div><p class="field-hint">${departmentNames.map(BRM.escape).join(' · ')||'No department assigned'}</p></div><div class="people-card-actions"><button class="button button-secondary button-small" data-edit-user="${u.UserID}">Edit account</button><a class="button button-secondary button-small" href="casting.html">Assign character</a>${deletable?`<button class="button button-danger button-small" data-quick-delete-user="${u.UserID}" data-quick-delete-username="${BRM.escape(u.Username)}" data-quick-delete-name="${BRM.escape(u.DisplayName||u.Username)}">Delete</button>`:''}</div></article>`;
  }).join('')}</div><div class="panel-inset hidden" data-user-filter-empty>No people match those filters.</div>`;
}
...
function applyAdminUserFilters(){
  const tbody=document.querySelector('[data-admin-users]');if(!tbody)return;
  const query=String(document.querySelector('[data-user-search]')?.value||'').trim().toLowerCase();
  const department=document.querySelector('[data-user-department]')?.value||'';
  ...
  rows.sort((a,b)=>{if(sort==='recent')return String(b.dataset.userCreated).localeCompare(String(a.dataset.userCreated));const key=sort==='department'?'userDepartmentName':sort==='grade'?'userGrade':sort==='status'?'userStatus':'userName';return String(a.dataset[key]||'').localeCompare(String(b.dataset[key]||''));}).forEach(row=>tbody.appendChild(row));
  let visible=0;rows.forEach(row=>{const ids=String(row.dataset.userDepartments||'').split('|').filter(Boolean);const matchesDepartment=!department||(department==='__none'?!ids.length:ids.includes(department));const matchesStatus=!status||row.dataset.userStatus===status;const matchesAccess=!access||(access==='admin'&&row.dataset.userAdmin==='true')||(access==='student'&&row.dataset.userAdmin!=='true')||(access==='password'&&row.dataset.userPassword==='true');const show=matchesDepartment&&matchesStatus&&matchesAccess&&(!query||String(row.dataset.userSearchText||'').includes(query));row.classList.toggle('hidden',!show);if(show)visible+=1;});
  const count=document.querySelector('[data-user-count]');if(count)count.textContent=`${visible} ${visible===1?'person':'people'}`;
  document.querySelector('[data-user-filter-empty]')?.classList.toggle('hidden',visible!==0);
}
window.filterAdminUsers=applyAdminUserFilters;

function quickDeleteAdminUser(button,data){
  const user=(data.users||[]).find(item=>String(item.UserID)===String(button.dataset.quickDeleteUser));if(!user)return;
  const modal=BRM.openModal(`<span class="eyebrow">Quick account removal</span><h2>Delete ${BRM.escape(button.dataset.quickDeleteName)}?</h2><div class="alert alert-error">This permanently removes the login, profile, sessions, and department requests. Production history remains in the audit record.</div><div class="form-actions"><button class="button button-secondary" data-cancel>Cancel</button><button class="button button-danger" data-confirm>Yes, delete account</button></div>`);
  modal.querySelector('[data-cancel]').onclick=()=>modal.closeModal();
  modal.querySelector('[data-confirm]').onclick=async event=>{const confirmButton=event.currentTarget;confirmButton.disabled=true;confirmButton.textContent='Deleting…';try{await BRM.api('deleteManagedUser',{userId:user.UserID,confirmUsername:user.Username});data.users=data.users.filter(item=>item.UserID!==user.UserID);document.querySelector(`[data-user-id="${CSS.escape(String(user.UserID))}"]`)?.remove();modal.closeModal();applyAdminUserFilters();BRM.toast('Student account deleted.');}catch(error){confirmButton.disabled=false;confirmButton.textContent='Yes, delete account';BRM.toast(error.message,'error');}};
}
